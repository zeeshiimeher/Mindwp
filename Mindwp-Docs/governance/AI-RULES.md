...
- `public/images/` = generated image outputs (read-only pipeline outputs)
...
### R16. Generated images are pipeline outputs.
- Images in `public/images/` are generated by `scripts/generators/image-generate.ts`.
- Do not manually edit, crop, or replace generated images.
- To regenerate: use `--force`, `--regenerate`, or `--fresh` flags.
- Image system config lives in `src/lib/image-system/config.ts`.
- Architecture doc: `Automatic-Image-Generation-System/IMAGE-SYSTEM-ARCHITECTURE.md`.
...
- run image generation scripts (`image-generate.ts`) in test mode
...
- run image generation in bulk mode (hits external APIs at scale)
...
| Manually editing generated images | Regenerate with `npx tsx scripts/generators/image-generate.ts --force` |
| Changing image overlay design outside config | Modify layout params in `featuredImage.ts` or config in `config.ts` |
