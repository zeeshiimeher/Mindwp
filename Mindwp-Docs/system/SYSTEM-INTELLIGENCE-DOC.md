...
6. After making fixes, log them to `reports/fix-log.json`.
...
All reports live in `/reports/`. They are generated by scripts and consumed by dashboards.
...
- **Generated by:** Manual entry or automated fix tracking
- **Used by:** Authority Dashboard (Fix History panel)
- **Contains:** Array of fix records — timestamp, slug, fix type, before/after scores
- **Purpose:** Tracks all content fixes over time. Enables learning which fixes improve scores most.

### fix-log.md

- **Generated by:** Derived from `fix-log.json`
- **Used by:** Human review
- **Contains:** Readable fix history
- **Purpose:** Quick review of recent fixes.

### session-log.json

- **Generated by:** Manual entry via `node scripts/dev/add-session-entry.mjs`
...
- Reports are **generated files** — do not edit them manually.
- The `fix-log.json` is the only report that accepts manual entries.
