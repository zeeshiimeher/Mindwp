...
export async function buildAuthorityMapArtifacts() {
  ...
  const formattedAuthorityMap = await prettier.format(content, {
    ...prettierConfig,
...
async function main() {
  ...
  const { formattedAuthorityMap, authorityReport, stats } = await buildAuthorityMapArtifacts();
  fs.writeFileSync(OUTPUT_FILE, formattedAuthorityMap, 'utf8');
