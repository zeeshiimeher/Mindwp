...
async function buildComponentDocsOutput() {
...
async function main() {
  const { outFile, content, count } = await buildComponentDocsOutput();

  fs.mkdirSync(path.dirname(outFile), { recursive: true });
  fs.writeFileSync(outFile, content, 'utf8');

  console.log(
    `[component-docs] Wrote ${count} component docs -> ${toPosix(path.relative(WORKSPACE_ROOT, outFile))}`
  );
}

module.exports = {
  buildComponentDocsOutput,
};
