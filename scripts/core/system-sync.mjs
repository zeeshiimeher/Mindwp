...
const phasePlaybookPath = path.join(root, 'Mindwp-Docs', 'PHASE-7-VISUAL-SYSTEM-AUDIT.md');
...
const phasePlaybookMd = fs.existsSync(phasePlaybookPath)
  ? fs.readFileSync(phasePlaybookPath, 'utf8')
  : '';
...
function extractDecisions(truthContent, indexContent, phaseContent) {
  const decisions = [];
  ...
  if (phaseContent.includes('**Execution order:** 7B → 7C → 7D → 7E')) {
    decisions.push({
      decision: 'Phase 7 execution order is locked',
      reason: 'Shared interaction and token cleanup must land before later component cleanup.',
      impact: 'Execute Phase 7 in the order 7B → 7C → 7D → 7E.',
      source: 'PHASE-7-VISUAL-SYSTEM-AUDIT.md',
    });
  }
...
const decisions = extractDecisions(truthMd, indexMd, phasePlaybookMd);
