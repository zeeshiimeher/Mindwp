...
const FULL_CTA_COUNT_SURFACE_PATTERNS = [
    '/renderers/',
    '/templates/',
    '/pages/',
    'src/screens/',
...
function extractPrimaryCtaBlocks(source: string) {
    return [...source.matchAll(/<PrimaryCTASection\b[\s\S]*?\/>/g)];
}

export function scanPrimaryCtaUsageFile(relativePath: string, source: string): Issue[] {
    const issues: Issue[] = [];
    const blocks = extractPrimaryCtaBlocks(source);
    const fullBlocks = blocks.filter(match => !/mode\s*=\s*['"]actions-only['"]/.test(match[0]));

    if (blocks.length > 0 && fullBlocks.length !== 1 && FULL_CTA_COUNT_SURFACE_PATTERNS.some(pattern => relativePath.includes(pattern))) {
        issues.push({
            severity: 'error',
            code: 'invalid_primary_cta_count',
            file: relativePath,
            line: lineOfIndex(source, blocks[0]?.index ?? 0),
    ...
    for (const block of blocks) {
        ...
        if (isActionsOnly) {
            issues.push({
                ...
                file: relativePath,
                line,
                message: 'PrimaryCTASection must not be used for action buttons. Use HeroActions or ActionButtons instead.',
            });
        }
        ...
        for (const legacyProp of ['system', 'pageType', 'slug', 'intent', 'position', 'tone', 'secondaryButtonCssPrefix']) {
    ...
    if (
        ...
        const match = source.match(/className\s*=\s*['"][^'"]*(cta__panel|cta__content|cta__actions)[^'"]*['"]/);
        issues.push({
            severity: 'error',
            code: 'custom_cta_wrapper_ui',
            file: relativePath,
            line: lineOfIndex(source, match?.index ?? 0),
            message: 'Custom CTA wrapper UI is not allowed. Use PrimaryCTASection directly.',
        });
    }
...
export function validateCorePrimaryCtaSources(
    ...
    if (!primaryCtaSectionSource.includes('allowSecondaryCTA?: true;')) {
        issues.push({
            ...
            file: relativePrimaryCtaSectionPath,
            line: 1,
            message: 'PrimaryCTASection secondary actions must remain an explicit allowSecondaryCTA opt-in.',
        });
    }
    ...
    if (!primaryCtaSectionSource.includes('Secondary CTA requires allowSecondaryCTA: true')) {
        issues.push({
            ...
            file: relativePrimaryCtaSectionPath,
            line: 1,
            message: 'PrimaryCTASection must fail loud when a secondary CTA is attempted without allowSecondaryCTA.',
        });
    }
