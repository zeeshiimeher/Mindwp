...
function scanButtonRule(): Issue[] {
  ...
  const industryCategoryTemplatePath = path.join(
    root,
    'src/domains/industries/templates/IndustryCategoryPageTemplate.tsx'
  );

  if (
    fs.existsSync(industryCategoryTemplatePath) &&
    fs.readFileSync(industryCategoryTemplatePath, 'utf8').includes('IndustrySubIndustriesSection')
  ) {
    pushIssue(issues, {
      rule: 'button-rule',
      domain: 'industries',
