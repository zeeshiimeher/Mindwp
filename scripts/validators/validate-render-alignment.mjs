...
function extractRendererOrder(rendererFilePath) {
    ...
    for (const identifier of sourceFile.getDescendantsOfKind(SyntaxKind.Identifier)) {
        ...
        if (
            ...
            (parent.getKind() === SyntaxKind.PropertyAccessExpression && parent.getNameNode() === identifier) ||
            (parent.getKind() === SyntaxKind.ShorthandPropertyAssignment && parent.getNameNode() === identifier) ||
...
function collectServiceIssues(issues, warnings) {
    ...
    const entryPattern = /['"]([^'"]+)['"]:\s*createServiceEntry\(\s*['"][^'"]+['"],\s*([A-Za-z0-9_]+),\s*([A-Za-z0-9_]+)\s*\)/g;
...
function resolveFeatureRendererFile(pageFile) {
    ...
    return rendererSpecifier ? importPathToFile(rendererSpecifier, path.dirname(pageFile), root) : null;
}
...
function collectFeatureIssues(issues, warnings) {
    ...
    const imports = parseImports(sourceText);
    const registryDir = path.dirname(registryPath);
    const entryPattern = /([A-Za-z0-9_-]+):\s*createFeatureEntry\(\s*['"]([^'"]+)['"],\s*([A-Za-z0-9_]+),\s*([A-Za-z0-9_]+),/g;
