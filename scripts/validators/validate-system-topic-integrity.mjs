...
async function main() {
  ...
  if (graph) {
    ...
    const orphanTopics = CANONICAL_TOPICS.filter(topic => (topicCounts.get(topic) ?? 0) === 0);
    const orphanSystems = CANONICAL_SYSTEMS.filter(system => (systemCounts.get(system) ?? 0) === 0);
