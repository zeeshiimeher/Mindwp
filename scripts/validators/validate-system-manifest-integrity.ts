...
const sourceCommand = 'node --import tsx/esm scripts/validators/validate-system-manifest-integrity.ts';
const logger = createLogger({ label: 'validate-system-manifest-integrity', mode: 'summary', rootDir: root });
...
function main() {
    ...
    const transientReportFiles = getTransientReportFiles().sort((left, right) => left.localeCompare(right));
    const actualReportFiles = fs.existsSync(reportsDir)
        ? [...listReportFiles(reportsDir), 'system-manifest-integrity-report.json'].sort((left, right) =>
