...
const ENGINE_PATH = path.join(ROOT, 'scripts/audit/visual-audit-v1.js');
...
export async function resolveAuditRoutes() {
  ...
  const sampledGroups = [
    ...
    ...resourceNodes.map(node => ({ ...node, family: 'resources', templateKind: 'resource-template' })),
    ...caseStudyNodes.map(node => ({ ...node, family: 'case-studies', templateKind: 'case-study-template' })),
...
function aggregateRepeatedBlocks(routeReports) {
  ...
  return [...blockMap.values()]
    ...
    .map(entry => ({
      ...
      averageScore: Math.round(entry.scores.reduce((sum, score) => sum + score, 0) / entry.scores.length),
      statuses: Object.fromEntries(entry.statuses),
    }))
    .sort((left, right) => left.averageScore - right.averageScore || left.name.localeCompare(right.name));
}
...
function aggregateCrossRouteComponents(routeReports) {
  ...
  return [...componentMap.values()]
    ...
    .filter(entry => entry.issues.length > 0 && entry.routeCount > 1 && entry.name !== 'div' && entry.name !== 'section')
...
function aggregateDesignSystem(routeReports) {
  ...
  spacingViolations.sort((left, right) => left.route.localeCompare(right.route) || left.component.localeCompare(right.component));
  colorViolations.sort((left, right) => left.route.localeCompare(right.route) || left.component.localeCompare(right.component));
...
function getWorstRoutes(routeReports) {
  return sortRouteReports(routeReports)
    .slice()
    .sort((left, right) => left.summary.averageScore - right.summary.averageScore || right.summary.issueCount - left.summary.issueCount)
    .slice(0, 10)
    .map(route => ({
      path: route.path,
...
function extractTopFindings(routeReports) {
  ...
  for (const finding of findings.sort((left, right) => compareSeverity(left, right) || left.route.localeCompare(right.route))) {
...
function sanitizeIssues(issues, limit = 5) {
  return sortIssueArray(dedupeIssues(issues)).slice(0, limit).map(issue => ({
    severity: issue.severity,
    code: issue.code,
    message: issue.message,
...
function formatMarkdown(report) {
  ...
  if (report.crossRouteComponents.length > 0) {
    ...
    for (const component of report.crossRouteComponents.slice(0, 15)) {
      lines.push(`- ${component.name}: ${component.issues.join('; ')} across ${component.routeCount} routes`);
    }
    lines.push('');
  }
  ...
  for (const violation of report.designSystem.spacingViolations.slice(0, 10)) {
    lines.push(`- ${violation.route} | ${violation.component} | ${violation.field} ${violation.value}px`);
  }
  ...
  for (const violation of report.designSystem.colorViolations.slice(0, 10)) {
    lines.push(`- ${violation.route} | ${violation.component} | ${violation.field} ${violation.value}`);
  }
  lines.push('');
  ...
  for (const route of report.routes) {
    ...
    lines.push(`- ${route.path} | score ${route.summary.averageScore} | issues ${route.summary.issueCount} | ${route.summary.status}${errorSuffix}`);
  }
...
export async function runVisualAudit(options = {}) {
          ...
          try {
            ...
            viewportReport.designSystem = {
              spacingViolations: [...(viewportReport.designSystem?.spacingViolations || [])].sort(
                (left, right) => left.component.localeCompare(right.component) || left.field.localeCompare(right.field)
              ),
            ...
            if (pageErrors.length > 0) {
              viewportReport.runtimeErrors = [...pageErrors].sort((left, right) => left.localeCompare(right));
            }
  ...
  const jsonPath = path.join(REPORTS_DIR, 'visual-audit.json');
  const markdownPath = path.join(REPORTS_DIR, 'visual-audit.md');
