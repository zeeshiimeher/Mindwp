...
export default function CRMAutomationRenderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <CRMAutomationHero hero={hero} ctaHref={cta.actions[0]?.href ?? '/contact'} />
      <CRMAutomationRecognitionSection />
      {faq ? <CRMAutomationFAQ faq={faq} /> : null}
      <CRMAutomationDecisionPanel cta={cta} />
    </main>
  );
}

function CRMAutomationHero({ hero, ctaHref }: { hero: Props['data']['hero']; ctaHref: string }) {
  return (
    <HeroFrame
      ariaLabel='CRM Automation hero'
      ...
      visual={<CRMAutomationSignalPanel visual={hero.visual} />}
    />
  );
}

function CRMAutomationSignalPanel({ visual }: { visual: Props['data']['hero']['visual'] }) {
...
function CRMAutomationRecognitionSection() {
  return (
    <SectionFrame
      id='website-handoff'
...
function CRMAutomationFAQ({ faq }: { faq: NonNullable<Props['data']['faq']> }) {
  return (
    <FAQSection
      title={faq.header.title}
      description={faq.header.description}
      items={faq.items.map((item, index) => ({
        id: `crm-automation-faq-${index}`,
        question: item.question,
        answer: item.answer,
      }))}
      tone='mist'
      variant='split'
      ariaLabel='CRM Automation FAQ'
    />
  );
}

function CRMAutomationDecisionPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
