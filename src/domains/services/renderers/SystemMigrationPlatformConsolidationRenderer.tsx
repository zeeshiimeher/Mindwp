...
const ARIA_HERO_DOT = 'System Migration -- page hero';
const ARIA_BOARD_DOT = 'Migration signals';
const ARIA_PATH_DOT = 'Consolidation path';
const ARIA_RISK_DOT = 'Risk check';
const ARIA_BRIDGE_DOT = 'System bridge';
const ARIA_FIT_DOT = 'Fit filter';
const ARIA_FAQ_DOT = 'Frequently asked questions';

const SOURCE_DOT = 'Signal';
const AGE_DOT = 'Age';
const STATE_WORKABLE_DOT = 'Migrate';
const STATE_CAUTION_DOT = 'Plan first';
const STATE_LEAVE_DOT = 'Leave alone';
const TIMING_DOT = 'Stage';
const GOOD_DOT = 'Right move';
const BAD_DOT = 'Wrong move';
const FIT_LABEL_DOT = 'Fit';
const NOT_FIT_LABEL_DOT = 'Not a fit';
const RULE_DOT = 'Rule';
const BRIDGE_MIG_DOT = 'Migration owns';
...
export function SystemMigrationPlatformConsolidationRenderer({ data, slug: _slug }: Props) {
  ...
  if (!primarySystem) throw new Error('[system-migration] Missing service system');
  const contactHref = buildServiceContactHref({ system: primarySystem, slug: data.slug });

  const migRows = handoffPlan.rows.filter(r => r.belongsTo === 'migration');
  const parentRows = handoffPlan.rows.filter(r => r.belongsTo === 'parent');

  return (
    <div className='migration-page feat-page'>
      <HeroFrame
        className='migration-hero feat-hero'
      ...
      >
        {(() => {
          requireHeadingTitle(migrationSignals.header.title, 'migrationSignals');
          return (
            <div className='feat-board__panel'>
              <header className='feat-board__head'>
                <span className='feat-board__title'>{migrationSignals.label}</span>
              </header>
              ...
              <ul className='feat-board__list'>
                {migrationSignals.sources.map(s => (
              ...
              <p className='feat-board__closing'>{migrationSignals.closing}</p>
            </div>
          );
        })()}
      </SectionFrame>
      ...
      >
        {(() => {
          requireHeadingTitle(consolidationPath.header.title, 'consolidationPath');
          return (
            <div className='feat-path__wrap'>
              <ol className='feat-path__steps'>
                {consolidationPath.steps.map(s => (
              ...
              <p className='feat-path__closing'>{consolidationPath.closing}</p>
            </div>
          );
        })()}
      </SectionFrame>
      ...
      <SectionFrame
        ...
        ariaLabel={ARIA_RISK_DOT}
      >
        {(() => {
          requireHeadingTitle(riskBoundaries.header.title, 'riskBoundaries');
          return (
            <div className='feat-ready__wrap'>
              <ul className='feat-ready__list'>
                {riskBoundaries.rows.map(r => (
              ...
              <p className='feat-ready__closing'>{riskBoundaries.closing}</p>
            </div>
          );
        })()}
      </SectionFrame>
      ...
      >
        {(() => {
          requireHeadingTitle(handoffPlan.header.title, 'handoffPlan');
          return (
            <div className='feat-bridge__wrap'>
              <div className='feat-bridge__columns'>
                <article className='feat-bridge__col feat-bridge__col--a'>
                  <header className='feat-bridge__head'>
                    <span className='feat-bridge__label'>{BRIDGE_MIG_DOT}</span>
                  </header>
              ...
              <p className='feat-bridge__rule'>
                <strong>{RULE_DOT}.</strong> {handoffPlan.rule}
              </p>
            </div>
          );
        })()}
      ...
      <DecisionPanel
        className='migration-cta feat-cta'
        heading={cta.heading}
