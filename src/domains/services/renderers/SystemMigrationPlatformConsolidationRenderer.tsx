...
export default function SystemMigrationPlatformConsolidationRenderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <SystemMigrationPlatformConsolidationHero
        hero={hero}
        ctaHref={cta.actions[0]?.href ?? '/contact'}
      />
      <SystemMigrationPlatformConsolidationRecognitionSection />
      {faq ? <SystemMigrationPlatformConsolidationFAQ faq={faq} /> : null}
      <SystemMigrationPlatformConsolidationDecisionPanel cta={cta} />
    </main>
  );
}

function SystemMigrationPlatformConsolidationHero({
  hero,
  ctaHref,
}: {
  ...
  return (
    <HeroFrame
      ariaLabel='System Migration & Platform Consolidation hero'
      ...
      visual={<SystemMigrationPlatformConsolidationSignalPanel visual={hero.visual} />}
    />
  );
}

function SystemMigrationPlatformConsolidationSignalPanel({
  visual,
}: {
...
function SystemMigrationPlatformConsolidationRecognitionSection() {
  return (
    <SectionShell
      id='website-handoff'
...
function SystemMigrationPlatformConsolidationFAQ({
  faq,
}: {
  faq: NonNullable<Props['data']['faq']>;
}) {
  return (
    <FAQSection
      ...
      items={faq.items.map((item, index) => ({
        id: `system-migration-platform-consolidation-faq-${index}`,
        question: item.question,
        answer: item.answer,
      }))}
      tone='mist'
      variant='split'
      ariaLabel='System Migration & Platform Consolidation FAQ'
    />
  );
}

function SystemMigrationPlatformConsolidationDecisionPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
...
export { SystemMigrationPlatformConsolidationRenderer };
