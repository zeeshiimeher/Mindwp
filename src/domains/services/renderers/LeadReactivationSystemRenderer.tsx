...
export default function LeadReactivationSystemRenderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <LeadReactivationSystemHero hero={hero} ctaHref={cta.actions[0]?.href ?? '/contact'} />
      <LeadReactivationSystemRecognitionSection />
      {faq ? <LeadReactivationSystemFAQ faq={faq} /> : null}
      <LeadReactivationSystemDecisionPanel cta={cta} />
    </main>
  );
}

function LeadReactivationSystemHero({
  hero,
  ctaHref,
}: {
  ...
  return (
    <HeroFrame
      ariaLabel='Lead Reactivation System hero'
      ...
      visual={<LeadReactivationSystemSignalPanel visual={hero.visual} />}
    />
  );
}

function LeadReactivationSystemSignalPanel({
  visual,
}: {
...
function LeadReactivationSystemRecognitionSection() {
  return (
    <SectionShell
      id='website-handoff'
...
function LeadReactivationSystemFAQ({ faq }: { faq: NonNullable<Props['data']['faq']> }) {
  return (
    <FAQSection
      title={faq.header.title}
      description={faq.header.description}
      items={faq.items.map((item, index) => ({
        id: `lead-reactivation-system-faq-${index}`,
        question: item.question,
        answer: item.answer,
      }))}
      tone='mist'
      variant='split'
      ariaLabel='Lead Reactivation System FAQ'
    />
  );
}

function LeadReactivationSystemDecisionPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
...
export { LeadReactivationSystemRenderer };
