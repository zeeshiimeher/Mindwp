...
// BricksBuilderRenderer — uses shared .builder-* CSS with .bricks-page accent override.
...
const ARIA_HERO_DOT = 'Bricks Builder -- page hero';
...
export function BricksBuilderRenderer({ data, slug: _slug }: Props) {
  ...
  if (!primarySystem) throw new Error('[bricks-builder] Missing service system');
  const contactHref = buildServiceContactHref({ system: primarySystem, slug: data.slug });

  return (
    <div className='bricks-page builder-page'>
      <HeroFrame
        className='bricks-hero builder-hero'
      ...
      <DecisionPanel
        className='bricks-cta builder-cta'
        heading={cta.heading}
