...
export default function Divi5Renderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <Divi5Hero hero={hero} ctaHref={cta.actions[0]?.href ?? '/contact'} />
      <Divi5RecognitionSection />
      {faq ? <Divi5FAQ faq={faq} /> : null}
      <Divi5DecisionPanel cta={cta} />
    </main>
  );
}

function Divi5Hero({ hero, ctaHref }: { hero: Props['data']['hero']; ctaHref: string }) {
  return (
    <HeroFrame
      ariaLabel='Divi 5 hero'
      ...
      visual={<Divi5SignalPanel visual={hero.visual} />}
    />
  );
}

function Divi5SignalPanel({ visual }: { visual: Props['data']['hero']['visual'] }) {
...
function Divi5RecognitionSection() {
  return (
    <SectionFrame
      id='website-handoff'
...
function Divi5FAQ({ faq }: { faq: NonNullable<Props['data']['faq']> }) {
  return (
    <FAQSection
      ...
      tone='mist'
      variant='split'
      ariaLabel='Divi 5 FAQ'
    />
  );
}

function Divi5DecisionPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
