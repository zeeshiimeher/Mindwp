...
export default function ConversionFunnelDecisionRenderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <ConversionFunnelDecisionHero hero={hero} ctaHref={cta.actions[0]?.href ?? '/contact'} />
      <ConversionFunnelDecisionRecognitionSection />
      {faq ? <ConversionFunnelDecisionFAQ faq={faq} /> : null}
      <ConversionFunnelDecisionPanel cta={cta} />
    </main>
  );
}

function ConversionFunnelDecisionHero({
  hero,
  ctaHref,
}: {
  ...
  return (
    <HeroFrame
      ariaLabel='Conversion Funnel vs Landing Page Decision hero'
      ...
      visual={<ConversionFunnelDecisionSignalPanel visual={hero.visual} />}
    />
  );
}

function ConversionFunnelDecisionSignalPanel({
  visual,
}: {
...
function ConversionFunnelDecisionRecognitionSection() {
  return (
    <SectionFrame
      id='website-handoff'
...
function ConversionFunnelDecisionFAQ({ faq }: { faq: NonNullable<Props['data']['faq']> }) {
  return (
    <FAQSection
      title={faq.header.title}
      description={faq.header.description}
      items={faq.items.map((item, index) => ({
        id: `conversion-funnel-decision-faq-${index}`,
        question: item.question,
        answer: item.answer,
      }))}
      tone='mist'
      variant='split'
      ariaLabel='Conversion Funnel vs Landing Page Decision FAQ'
    />
  );
}

function ConversionFunnelDecisionPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
...
export { ConversionFunnelDecisionRenderer };
