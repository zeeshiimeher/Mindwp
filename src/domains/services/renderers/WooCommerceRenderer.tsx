...
export default function WooCommerceRenderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <WooCommerceHero hero={hero} ctaHref={cta.actions[0]?.href ?? '/contact'} />
      <WooCommerceRecognitionSection />
      {faq ? <WooCommerceFAQ faq={faq} /> : null}
      <WooCommerceDecisionPanel cta={cta} />
    </main>
  );
}

function WooCommerceHero({ hero, ctaHref }: { hero: Props['data']['hero']; ctaHref: string }) {
  return (
    <HeroFrame
      ariaLabel='WooCommerce hero'
      ...
      visual={<WooCommerceSignalPanel visual={hero.visual} />}
    />
  );
}

function WooCommerceSignalPanel({ visual }: { visual: Props['data']['hero']['visual'] }) {
...
function WooCommerceFAQ({ faq }: { faq: NonNullable<Props['data']['faq']> }) {
  return (
    <FAQSection
      title={faq.header.title}
      description={faq.header.description}
      items={faq.items.map((item, index) => ({
        id: `woocommerce-faq-${index}`,
        question: item.question,
        answer: item.answer,
      }))}
      tone='mist'
      variant='split'
      ariaLabel='WooCommerce FAQ'
    />
  );
}

function WooCommerceDecisionPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
