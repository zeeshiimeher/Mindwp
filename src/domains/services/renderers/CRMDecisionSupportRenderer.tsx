...
export default function CRMDecisionSupportRenderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <CRMDecisionSupportHero hero={hero} ctaHref={cta.actions[0]?.href ?? '/contact'} />
      <CRMDecisionSupportRecognitionSection />
      {faq ? <CRMDecisionSupportFAQ faq={faq} /> : null}
      <CRMDecisionSupportPanel cta={cta} />
    </main>
  );
}

function CRMDecisionSupportHero({
  hero,
  ctaHref,
}: {
  ...
  return (
    <HeroFrame
      ariaLabel='CRM Decision Support hero'
      ...
      visual={<CRMDecisionSupportSignalPanel visual={hero.visual} />}
    />
  );
}

function CRMDecisionSupportSignalPanel({ visual }: { visual: Props['data']['hero']['visual'] }) {
...
function CRMDecisionSupportRecognitionSection() {
  return (
    <SectionFrame
      id='website-handoff'
...
function CRMDecisionSupportFAQ({ faq }: { faq: NonNullable<Props['data']['faq']> }) {
  return (
    <FAQSection
      title={faq.header.title}
      description={faq.header.description}
      items={faq.items.map((item, index) => ({
        id: `crm-decision-support-faq-${index}`,
        question: item.question,
        answer: item.answer,
      }))}
      tone='mist'
      variant='split'
      ariaLabel='CRM Decision Support FAQ'
    />
  );
}

function CRMDecisionSupportPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
...
export { CRMDecisionSupportRenderer };
