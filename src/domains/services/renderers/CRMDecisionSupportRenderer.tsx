...
const ARIA_HERO_DOT = 'CRM Decision -- page hero';
...
export function CRMDecisionSupportRenderer({ data, slug: _slug }: Props) {
  ...
  if (!primarySystem) throw new Error('[crm-decision] Missing service system');
  const contactHref = buildServiceContactHref({ system: primarySystem, slug: data.slug });

  return (
    <div className='crm-decision-page decision-page'>
      <HeroFrame
        className='crm-decision-hero decision-hero'
      ...
      <DecisionPanel
        className='crm-decision-cta decision-cta'
        heading={cta.heading}
