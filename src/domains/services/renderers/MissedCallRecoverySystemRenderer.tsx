...
export default function MissedCallRecoverySystemRenderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <MissedCallRecoverySystemHero hero={hero} ctaHref={cta.actions[0]?.href ?? '/contact'} />
      <MissedCallRecoverySystemRecognitionSection />
      {faq ? <MissedCallRecoverySystemFAQ faq={faq} /> : null}
      <MissedCallRecoverySystemDecisionPanel cta={cta} />
    </main>
  );
}

function MissedCallRecoverySystemHero({
  hero,
  ctaHref,
}: {
  ...
  return (
    <HeroFrame
      ariaLabel='Missed Call Recovery System hero'
      ...
      visual={<MissedCallRecoverySystemSignalPanel visual={hero.visual} />}
    />
  );
}

function MissedCallRecoverySystemSignalPanel({
  visual,
}: {
...
function MissedCallRecoverySystemRecognitionSection() {
  return (
    <SectionFrame
      id='website-handoff'
...
function MissedCallRecoverySystemFAQ({ faq }: { faq: NonNullable<Props['data']['faq']> }) {
  return (
    <FAQSection
      title={faq.header.title}
      description={faq.header.description}
      items={faq.items.map((item, index) => ({
        id: `missed-call-recovery-system-faq-${index}`,
        question: item.question,
        answer: item.answer,
      }))}
      tone='mist'
      variant='split'
      ariaLabel='Missed Call Recovery System FAQ'
    />
  );
}

function MissedCallRecoverySystemDecisionPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
...
export { MissedCallRecoverySystemRenderer };
