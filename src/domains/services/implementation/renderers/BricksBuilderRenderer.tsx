...
export default function BricksBuilderRenderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <BricksBuilderHero hero={hero} ctaHref={cta.actions[0]?.href ?? '/contact'} />
      <BricksBuilderRecognitionSection />
      {faq ? <BricksBuilderFAQ faq={faq} /> : null}
      <BricksBuilderDecisionPanel cta={cta} />
    </main>
  );
}

function BricksBuilderHero({ hero, ctaHref }: { hero: Props['data']['hero']; ctaHref: string }) {
  return (
    <HeroFrame
      ariaLabel='Bricks Builder hero'
      ...
      visual={<BricksBuilderSignalPanel visual={hero.visual} />}
    />
  );
}

function BricksBuilderSignalPanel({ visual }: { visual: Props['data']['hero']['visual'] }) {
...
function BricksBuilderRecognitionSection() {
  return (
    <SectionShell
      id='website-handoff'
...
function BricksBuilderFAQ({ faq }: { faq: NonNullable<Props['data']['faq']> }) {
  return (
    <FAQSection
      title={faq.header.title}
      description={faq.header.description}
      items={faq.items.map((item, index) => ({
        id: `bricks-builder-faq-${index}`,
        question: item.question,
        answer: item.answer,
      }))}
      tone='mist'
      variant='split'
      ariaLabel='Bricks Builder FAQ'
    />
  );
}

function BricksBuilderDecisionPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
