...
export default function WordPressDevelopmentRenderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <WordPressDevelopmentHero hero={hero} ctaHref={cta.actions[0]?.href ?? '/contact'} />
      <WordPressDevelopmentRecognitionSection />
      {faq ? <WordPressDevelopmentFAQ faq={faq} /> : null}
      <WordPressDevelopmentDecisionPanel cta={cta} />
    </main>
  );
}

function WordPressDevelopmentHero({
  hero,
  ctaHref,
}: {
  ...
  return (
    <HeroFrame
      ariaLabel='WordPress Development hero'
      ...
      visual={<WordPressDevelopmentSignalPanel visual={hero.visual} />}
    />
  );
}

function WordPressDevelopmentSignalPanel({ visual }: { visual: Props['data']['hero']['visual'] }) {
...
function WordPressDevelopmentRecognitionSection() {
  return (
    <SectionShell
      id='website-handoff'
...
function WordPressDevelopmentFAQ({ faq }: { faq: NonNullable<Props['data']['faq']> }) {
  return (
    <FAQSection
      title={faq.header.title}
      description={faq.header.description}
      items={faq.items.map((item, index) => ({
        id: `wordpress-development-faq-${index}`,
        question: item.question,
        answer: item.answer,
      }))}
      tone='mist'
      variant='split'
      ariaLabel='WordPress Development FAQ'
    />
  );
}

function WordPressDevelopmentDecisionPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
...
export { WordPressDevelopmentRenderer };
