...
export function CalendarsRenderer({ data }: Props) {
  const { hero, cta } = data;
  const faq = data.faq;

  return (
    <main>
      <CalendarsHero hero={hero} ctaHref={cta.actions[0]?.href ?? '/contact'} />
      <CalendarsRecognitionSection />
      {faq ? <CalendarsFAQ faq={faq} /> : null}
      <CalendarsDecisionPanel cta={cta} />
    </main>
  );
}

function CalendarsHero({ hero, ctaHref }: { hero: Props['data']['hero']; ctaHref: string }) {
  return (
    <HeroFrame
      ariaLabel='Calendars hero'
      ...
      visual={<CalendarsSignalPanel visual={hero.visual} />}
    />
  );
}

function CalendarsSignalPanel({ visual }: { visual: Props['data']['hero']['visual'] }) {
...
function CalendarsRecognitionSection() {
  return (
    <SectionFrame
      id='website-handoff'
...
function CalendarsFAQ({ faq }: { faq: NonNullable<Props['data']['faq']> }) {
  return (
    <FAQSection
...
function CalendarsDecisionPanel({ cta }: { cta: Props['data']['cta'] }) {
  return (
    <DecisionPanel
      heading={{
...
export default CalendarsRenderer;
