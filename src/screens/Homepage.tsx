  ...
  return (
    <div className='infrastructure-gaps-container-1 l-container'>
      ...
      <div className='infrastructure-gaps-grid'>
        {infrastructureGapsData.problems.map(problem => (
          <ProblemSolutionSplitCard
            key={`${problem.title}-${problem.impact}`}
            icon={problem.icon}
      ...
      <div className='framework-section-split'>
        ...
        <div className='framework-section-features-wrapper'>
          <div className='framework-section-features-list'>
            {smartWebsiteFrameworkData.principles.map(item => (
              <IconBenefitCard
        ...
        <div className='trust-section__cards'>
          <div className='trust-section__card'>
            ...
            <ul className='trust-section__list'>
              {trustFoundationsData.strongFitItems.map(item => (
                <li key={`${item.title}-${item.description}`} className='trust-section__list-item'>
          ...
          <div className='trust-section__card'>
            ...
            <ul className='trust-section__list'>
              {trustFoundationsData.notDesignedItems.map(item => (
                <li key={`${item.title}-${item.description}`} className='trust-section__list-item'>
                  <div className='trust-section__item-title'>{item.title}</div>
                  <div className='trust-section__item-description'>{item.description}</div>
                </li>
  ...
  return (
    <div className='c-system-capabilities__visual c-system-capabilities__visual--calls'>
      <svg viewBox='0 0 320 180' className='c-system-capabilities__visual-svg' aria-hidden='true'>
        ...
        {[0, 1, 2].map(callSlot => (
          <g key={`incoming-call-${callSlot}`} transform={`translate(34 ${52 + callSlot * 28})`}>
            <rect width='98' height='18' rx='9' fill='rgba(255,255,255,0.88)' />
            <circle cx='14' cy='9' r='4' fill='currentColor' opacity={0.35 + callSlot * 0.12}>
              <animate
                attributeName='r'
                values='4;6;4'
                dur={`${1.5 + callSlot * 0.3}s`}
        ...
        {[0, 1, 2].map(callbackSlot => (
          <g
            key={`callback-${callbackSlot}`}
            transform={`translate(188 ${66 + callbackSlot * 22})`}
          >
            ...
            <rect
              ...
              rx='4'
              fill='currentColor'
              opacity={0.28 + callbackSlot * 0.08}
            >
              <animate
                attributeName='opacity'
                values='0.24;0.62;0.24'
                dur={`${1.8 + callbackSlot * 0.2}s`}
                repeatCount='indefinite'
              />
            </rect>
          </g>
      ...
      <div className='c-system-capabilities__visual-grid'>
        {stats.map(stat => (
          <div key={`${stat.label}-${stat.value}`} className='c-system-capabilities__visual-stat'>
  ...
  return (
    <div className='c-system-capabilities__visual c-system-capabilities__visual--leads'>
      <svg viewBox='0 0 320 180' className='c-system-capabilities__visual-svg' aria-hidden='true'>
        ...
        {[0, 1, 2, 3].map(leadRow => (
          <g key={`lead-row-${leadRow}`} transform={`translate(40 ${78 + leadRow * 20})`}>
            <rect
              width='240'
              height='14'
              rx='7'
              fill={leadRow === 1 ? 'rgba(255,255,255,0.98)' : 'rgba(255,255,255,0.8)'}
            />
            ...
            <rect
              ...
              height='8'
              rx='4'
              fill={leadRow === 1 ? 'currentColor' : 'rgba(255,255,255,0.96)'}
              opacity={leadRow === 1 ? 0.32 : 1}
            >
              {leadRow === 1 ? (
                <animate
                  attributeName='opacity'
        ...
        {[0, 1, 2].map(visibilityRow => (
          ...
          >
            ...
            <rect
              x='10'
              y='4'
              width={visibilityRow === 0 ? 56 : visibilityRow === 1 ? 42 : 32}
              height='4'
              rx='2'
              fill='currentColor'
              opacity='0.16'
                  ...
                  <div className={`${BLOCK}__content`}>
                    ...
                    <ul className={`${BLOCK}__benefits`}>
                      {component.benefits.map(benefit => (
                        <li key={benefit} className={`${BLOCK}__benefit`}>
      ...
      <div className='infrastructure-layers-grid'>
        ...
        <div className='infrastructure-layers-foundation-card'>
          ...
          <ul className='infrastructure-layers-foundation-list'>
            {infrastructureLayersData.foundation.checklist.map(item => (
        ...
        <div className='infrastructure-layers-stack'>
          {infrastructureLayersData.layers.map((layer, index) => (
            <IconBenefitCard
              key={`${layer.title}-${layer.description}`}
              title={layer.title}
