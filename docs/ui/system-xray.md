...
# MindWP Full System Refactor

Date: 2026-05-04

Status: Approved planning blueprint. This file defines the decided business, system, UI, component, variant, and migration direction for the MindWP full-system refactor. It is planning authority for future implementation, not permission to execute runtime changes automatically.

Planning posture:

- Preserve detailed component-by-component guidance
- Merge useful detail from legacy audit documents
- Decisions inside this document are approved planning direction unless explicitly marked Deprecated
- Keep higher-level governance in FOUNDATION, CONTENT, CONVERSION, GRAPH, DESIGN, SYSTEM-RULES, and WORKFLOW
- Treat code as the source of truth when docs and code conflict
- Use this file as the operating guide for future scoped system refactor work
...
---
### Phase 0 Safe Alignment Before Phase 1

Phase 0 is safe alignment work. It is allowed to make small runtime cleanup edits, but it is NOT the full component redesign and it must NOT create new components.

Phase 0 safe alignment tasks:

1. Remove or rewrite public GoHighLevel mention in Smart Website data.
2. Rename `FitCheckSection` to `QualificationSection` where safe.
3. Update imports, exports, renderers, and direct validator/test references caused by the rename.
4. Replace anchor `FitCheckSection` / `two-column` usage with `QualificationSection` / `fit-filter` where safe.
5. Clean deprecated anchor variant names only where the existing component can still render without creating a missing component.
6. Identify GridCardsSection `signal-board` usages without badge/status data and either safely switch them to `feature-grid` or defer them to Phase 1.
7. Update `system-xray.md` with Phase 0 completion status and Phase 1 deferrals after the safe alignment work.

Phase 0 rule:

- Phase 0 may clean existing naming, imports, exports, direct references, and safe deprecated variant usage.
- Phase 0 may remove public GoHighLevel mentions from public-facing data.
- Phase 0 must not create new approved components.
- Phase 0 must not add new top-third anchor content.
- Phase 0 must not fully migrate Smart Website or Local SEO to the final approved mapping if that requires missing components.
- Phase 0 must not invent staged leaks, handoffs, criteria rows, signal states, metrics, proof, testimonials, guarantees, or capabilities.

Phase 0 execution boundary:

Allowed in Phase 0:

- public GoHighLevel cleanup
- `FitCheckSection` -> `QualificationSection` rename
- direct imports/exports/usages cleanup caused by the rename
- direct validators/tests reference cleanup caused by the rename
- safe deprecated variant cleanup where existing components already support the approved replacement
- docs update to record Phase 0 completion and Phase 1 deferrals

Not allowed in Phase 0:

- creating `JourneyLeakMapSection`
- creating `ServiceBridgeSection`
- creating `CriteriaComparisonSection`
- creating `AuthoritySignalMapSection`
- creating any other missing approved component
- adding new staged leak content
- adding new system handoff content
- adding new criteria comparison rows
- adding new signal-family/state content
- full anchor renderer migration to missing components
- supporting service page migration
- validator/test redesign

Phase 1 deferred work:

1. Lock user-approved content for Smart Website `JourneyLeakMapSection`.
2. Lock user-approved content for Smart Website `ServiceBridgeSection`.
3. Decide whether Smart Website `sections.types` becomes true capability content or is replaced/removed.
4. Decide keep/merge/remove for Smart Website extra legacy sections: `businessSizes`, `concerns`, `technologies`.
5. Lock user-approved content for Local SEO `CriteriaComparisonSection`.
6. Decide whether Local SEO `sections.why` and `sections.integrations` merge into one `AuthoritySignalMapSection` or remain separate content sources.
7. Create missing anchor-specific components after content decisions are locked.
8. Migrate Smart Website and Local SEO renderers to the final approved mapping.
9. Align validators and tests only after runtime contracts exist.

Final rule:
If Phase 0 work requires a missing component, new top-third content, or deeper renderer redesign, stop and defer it to Phase 1 instead of forcing it.
...
## 3.5C Anchor Data + Section Contract Reality Check

Purpose:

- Capture the current repo reality for Smart Website Systems and Local SEO Authority before implementation.
- Separate direct mapping, light reshape, moderate reshape, and user-approved content gaps.
- Prevent Copilot from inventing props, sections, claims, metrics, or filler copy when current data does not support the approved mapping.
- Define Phase 0 work before Phase 1 component implementation begins.

Reality scope:

- Planning authority: `docs/ui/system-xray.md`
...
- The approved new anchor components do not exist yet in `src/components/sections`.
- Both anchor renderers still use legacy component names or deprecated variant names.
- Smart Website and Local SEO data is strong enough for many direct maps and light reshapes.
- Some approved top-third sections require new user-approved content because current data does not honestly express the approved pattern.
- Validators and tests should not be aligned until component and data contracts are real in runtime.

### Smart Website Systems Reality

| Approved Position | Approved Component | Current Runtime | Current Data Source | Data Fit | Content Risk | Decision |
|---|---|---|---|---|---|---|
| Recognition | HeroSplitSection visualType `system-feed` | HeroSplitSection variant `operations` | `data.hero` | Direct Map | Low | Rename variant concept to visualType; no new content needed |
| Diagnosis | JourneyLeakMapSection | GridCardsSection | `sections.value` | New User-Approved Content Needed | High | Current data is peer symptoms, not staged leak map |
| Comparison | BeforeAfterSection `split-panel` | BeforeAfterSection | `sections.comparison` | Direct Map | Low | Keep content, align variant naming |
| System Handoff | ServiceBridgeSection | Not Present | None | New User-Approved Content Needed | High | Needs explicit from/to/handoff/boundary content |
| Scope | ScopeSection `grouped-scope` or `service-map` | GridCardsSection | `sections.included` | Moderate Reshape | Medium | Existing inclusions need grouping/areas/outcomes |
| Capability | GridCardsSection `feature-grid` | GridCardsSection | `sections.types` | New User-Approved Content Needed | High | Current types are audience/use-case framing, not peer capabilities |
| System Layers | LayerStackSection `stack` | LayerStackSection `interactive-stack` | `sections.coreLayer` | Light Reshape | Low | Keep meaning, clean deprecated naming |
| Proof | ProofStorySection `before-change-after` | ProofStorySection | `sections.proof` | Direct Map | Low | No metric proof unless real metric data exists |
| Visual Evidence | ImageStorySection `evidence-photo` or `system-visual` | ImageStorySection `operational-photo` | `sections.visibilityFoundations` | Light Reshape | Medium | Existing content usable; visual proof role must be clarified |
| Process | ProcessStepsSection `timeline` | ProcessStepsSection | `sections.process` | Direct Map | Low | Keep content |
| Qualification | QualificationSection `fit-filter` | FitCheckSection `two-column` | `sections.qualification` | Direct Map | Low | Rename/migrate only |
| FAQ | AccordionFAQSection `single-column` | AccordionFAQSection | `sections.faq` | Direct Map | Low | Keep content |
| CTA | PrimaryCTASection `soft-panel` | PrimaryCTASection | `data.cta` | Direct Map | Low | Keep content |

Smart Website content gaps requiring user approval:

- JourneyLeakMapSection staged leak-path content.
- ServiceBridgeSection handoff content.
- Capability section if `sections.types` should be replaced by true peer capabilities.

Smart Website safe reshape opportunities:

- Hero visualType cleanup.
- Comparison direct map.
- Scope grouping from included content.
- LayerStackSection naming cleanup.
- ImageStorySection naming/role cleanup.
- QualificationSection rename from FitCheckSection.
- FAQ and CTA remain structurally aligned.

Smart Website extra legacy sections needing keep/merge/remove decision:

- `sections.businessSizes`
- `sections.concerns`
- `sections.technologies`

### Local SEO Authority Reality

| Approved Position | Approved Component | Current Runtime | Current Data Source | Data Fit | Content Risk | Decision |
|---|---|---|---|---|---|---|
| Recognition | HeroSplitSection visualType `signal-grid` | HeroSplitSection variant `visibility` | `data.hero` | Light/Moderate Reshape | Low/Medium | Move from deprecated hero variant to visualType; regroup if needed |
| Criteria Comparison | CriteriaComparisonSection | Not Present | None | New User-Approved Content Needed | High | Needs explicit criteria rows for package thinking vs authority system thinking |
| Signal Map | AuthoritySignalMapSection | LayerStackSection `signal-map` | `sections.why`, possibly `sections.integrations` | Moderate Reshape | Medium | Meaning exists, but families/states are not explicit |
| Comparison | BeforeAfterSection `split-panel` | BeforeAfterSection `scorecard` | `sections.comparison` | Direct Map | Low | Keep data, remove deprecated scorecard naming |
| Diagnosis | GridCardsSection `signal-board` | GridCardsSection `signal-board` | `sections.misconceptions` | Direct Map | Low | Current badge/status data materially supports signal-board |
| Scope | ScopeSection `grouped-scope` or `service-map` | GridCardsSection `feature-grid` | `sections.scopeSection` | Light Reshape | Medium | Current service list can likely map to service-map |
| Process | ProcessStepsSection `timeline` | ProcessStepsSection | `sections.processSection` | Direct Map | Low | Keep content |
| Proof | ProofStorySection `before-change-after` | ProofStorySection | `sections.proof` | Direct Map | Low | No metric proof unless real metric data exists |
| Qualification | QualificationSection `fit-filter` | FitCheckSection `two-column` | `sections.qualification` | Direct Map | Low | Rename/migrate only |
| FAQ | AccordionFAQSection `single-column` | AccordionFAQSection | `sections.faqSection` | Direct Map | Low | Keep content |
| CTA | PrimaryCTASection `soft-panel` | PrimaryCTASection | `data.cta` | Direct Map | Low | Keep content |

Local SEO content gaps requiring user approval:

- CriteriaComparisonSection criteria rows.
- AuthoritySignalMapSection family/state structure if it cannot be honestly derived from `sections.why` and `sections.integrations`.

Local SEO safe reshape opportunities:

- Hero visualType cleanup.
- Comparison scorecard cleanup into split-panel.
- Scope reshape into ScopeSection.
- QualificationSection rename from FitCheckSection.
- Process, FAQ, proof, and CTA remain structurally aligned.

Local SEO extra legacy section needing keep/merge/remove decision:

- `sections.integrations` as a second signal-like section.
...
- `src/domains/services/data/smart-website-systems.ts`
  - `smartWebsitesFaqItems`
  - FAQ question: `Do we need a specific CRM?`
  - Answer mentions GoHighLevel.

Decision:

- Future scoped cleanup must remove public GoHighLevel mention.
- Replacement wording should use generic white-label language such as CRM platform, lead handling system, follow-up system, or automation layer depending on context.
- Do not remove legitimate public platform mentions such as WordPress, Google, Google Business Profile, Google Search Console, or Google Ads unless a separate content decision requires it.


## 3.6 System Usage Reality (Reference)

Purpose:
- capture real usage vs system rules
- highlight common misuse patterns
- guide migration decisions

Known Misuse Patterns:

- GridCardsSection used for:
  - grouped scope → should be ScopeSection
  - signal mapping → should be AuthoritySignalMapSection

- LayerStackSection used for:
  - signal relationships → wrong component

- ProcessStepsSection used for:
  - non-linear explanation → should be DecisionFrameworkSection

- BeforeAfterSection:
  - scorecard used without real criteria → invalid

- ProofStorySection:
  - metric-only usage → invalid

Rule:

If real usage conflicts with component definition:
→ component definition MUST win
→ usage must be corrected

## 3.7 Component Selection Heuristics (Reference)
...
# 4. Visual System (Phase X Authority)
...
# 5. Component System (APPROVED PLANNING AUTHORITY)
...
## 5.6 2026-05-01 Audit Extraction

Status: Approved extraction from the 2026-05-01 GPT-5.5 component audit. Use this as planning and future execution guidance, while still verifying exact current code before editing files.

Useful decisions carried forward:

- QualificationSection replaces FitCheckSection.
- Variants must be structural, not cosmetic.
- GridCardsSection must not absorb every mid-page pattern.
- Tier-1 pages need unique top-third structural patterns.
- Smart Website needs a leakage-map pattern.
- Local SEO needs an authority-signal-map pattern.
- ScopeSection should own grouped scope, not GridCardsSection.
- RelatedContentSection should only render graph-owned resolved candidates.
- `/dev/component-system` should be used as the visual execution and QA surface.

Variant reality check:

- HeroSplitSection uses `visualType`, not hero variants.
- GridCardsSection keeps `diagnostic-grid`, `feature-grid`, and `signal-board`; `signal-board` is valid only when badge/status data is structurally used.
- ScenarioMapSection keeps `primary-scenario` and `environment-map`; `scenario-grid` and `use-case-grid` are deprecated patterns.
- CaseStudyNarrativeSection keeps `narrative-flow` and `proof-breakdown`; generic story/card variants are not approved.
- DecisionFrameworkSection keeps `principle-steps` and `checklist-framework`; `FrameworkStepsSection` is deprecated naming and process/checklist UI variants are not approved.
- ScopeSection keeps `grouped-scope` and `service-map`; feature-grid-like usage is deprecated.
- ImageStorySection keeps `evidence-photo` and `system-visual`; `operational-photo`, `visual-panel`, and `split-evidence` are deprecated names.
- `scorecard` is deprecated unless BeforeAfterSection gains real metric/card criteria structure.
- `decision-cards` is deprecated.
- `cycle` is deprecated.
- `metric-story` is deprecated.
- `split-card` is deprecated unless PrimaryCTASection gives supports/actions a structurally separate region.
- `signal-map` inside LayerStackSection is deprecated.

Component responsibility boundaries:

- GridCardsSection = independent peer cards only.
- LayerStackSection = ordered layered explanation.
- ProcessStepsSection = linear process flow.
- ScopeSection = grouped inclusion/scope lists.
- ProofStorySection = before/change/after proof.
- QualificationSection = fit / not-fit filtering.
- RelatedContentSection = resolved graph candidates only.

Duplication risks:

- GridCardsSection, ScopeSection, and LayerStackSection overlap unless their data contracts are strict.
- BeforeAfterSection and QualificationSection can look visually similar, but comparison and qualification are different section intents.
- ProcessStepsSection and LayerStackSection both use ordered sequences, but ProcessStepsSection owns flow while LayerStackSection owns always-visible layers.
- GridCardsSection becomes too generic if it absorbs diagnostics, features, scope, concerns, technologies, and misconceptions without stricter rules.

Dev visualizer planning guidance:

- Route: `/dev/component-system`.
- Dev-gated only.
...
- Do not introduce fake marketing copy.
...
# 7. Validation System (Future Implementation Layer)
...
# 8. Phases (Reference Only)

## 8.1 Phase 3 — Audit
## 8.2 Phase 4 — Cleanup
## 8.3 Phase 6 — Improvements

NOTE:
These are NOT authority layers.

# 9. Implementation Gap Backlog

Backlog / verify against current code before executing.

Purpose:
- Track real-world implementation gaps
- Ensure system and code remain aligned

Rules:

- This section is informational only
- verify each item against current code before acting
- Validators must NOT read this
- backlog notes are not guaranteed current truth

Example tracking:

- LayerStackSection:
  - CSS class mismatch (`layer-stack__index` vs `layer-stack__card-index`)

- GridCardsSection:
  - `signal-board` requires badge/status but not enforced in code

- GridCardsSection:
  - `.grid-cards--signal-board .grid-card[data-status='risk']` does not match JSX class `grid-cards__item`, and no `data-status` is emitted

- ScopeSection:
  - blank item strings should be validated and rejected

- RelatedContentSection:
  - error references `groups` instead of `items`

- QualificationSection:
  - verify remaining FitCheck naming, imports, validators, and tests before executing any migration

- PrimaryCTASection:
  - keep current CTA ownership; any CTASection rename is a parked future migration only

If mismatch exists:
→ must be fixed in code
→ system-xray remains authority

---

# 10. Future Execution Appendices
...
- Phase 0 Safe Alignment must not create new components or new top-third content
- if a Phase 0 cleanup requires a missing component, defer it to Phase 1
...
- Phase 0 handles safe legacy alignment only; Phase 1 handles new component creation and anchor renderer migration
