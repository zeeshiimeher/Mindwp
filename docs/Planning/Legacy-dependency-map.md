...
> Latest baseline: Homepage, Smart Website Systems, and Local SEO Authority rebuilt. Service detail renderers are staged with clean new-system skeletons. CaseStudyTemplate no longer owns related or final CTA rendering. All rebuilt renderers use SectionFrame.
...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------- | ----------------------- | ------------------------------------------------------------ |
| `src/components/sections/PrimaryCTASection.tsx` | About.tsx, Contact.tsx, BlogPostTemplate, BlogFooterCTA, case-studies/pages/index.tsx, Features pages + 7 renderers, Industries templates + pages, Resources hub/template, `/services` listing page, dev/cta-label-contract          | all unrebuilt domains + About/Contact + service listing | Quarantine delete-later | When all consuming pages are rebuilt and no longer import it |
...
| `src/components/reusable/` (entire folder)      | Blog templates/UI, Resources templates/pages, CaseStudyTemplate (old visual sections only), Features renderers (7), Industries templates, `/services` listing page, About, Contact, dev dashboards, ClusterPageLayout, ActionButtons | All unrebuilt domains + shared system components        | Quarantine delete-later | When all consuming pages/components are rebuilt              |
...
Service detail renderers are staged clean and covered by `validate-legacy-quarantine.mjs`.

Remaining old service-domain consumer:

- `src/domains/services/pages/index.tsx` — `/services` listing page imports reusable components and `PrimaryCTASection`.
