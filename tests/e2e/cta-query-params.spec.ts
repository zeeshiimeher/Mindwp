...
test('all major page-family CTAs include valid canonical query params', async ({ page }) => {
  ...
  for (const entry of pages) {
    ...
    for (const href of hrefs) {
      ...
      expect(
        canonicalSystems.has(system),
        `Non-canonical CTA system on ${entry.path}: ${system}`
      ).toBe(true);
      expect(
        /^[a-z-]+\/[a-z0-9-]+$/.test(source),
        `Invalid CTA source on ${entry.path}: ${source}`
      ).toBe(true);
    ...
    expect(
      sources.has(entry.expectedSource),
      `Missing page-specific CTA source on ${entry.path}`
    ).toBe(true);
  }
});
