...
const APPROVED_PUBLIC_TOPICS = new Set(
    (((topicAuthorityScoresReport as { data?: { scores?: TopicAuthorityScoreEntry[] } }).data?.scores) ?? [])
...
function resolveTopicClassification(kind: IndexingPolicyKind, path: string): ResolvedIndexingPolicy {
...
export function resolveIndexingPolicy(kind: IndexingPolicyKind, routePath: string): ResolvedIndexingPolicy {
    ...
    return resolveKindClassification(kind, path) ?? buildPolicy('utility', 'fallback', {
        index: false,
        follow: false,
        disallow: true,
    });
